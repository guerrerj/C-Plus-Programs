...
bool FlappyBird::OnUserUpdate(float fElapsedTime)
{
	if (bResetGame)
	{
		bHasCollided = false;
	...
	if (bHasCollided)
	{
		//Do not do anything until user releases space
	...
	else
	{
		...
		//Display 
		Fill(0, 0, ScreenWidth(), ScreenHeight(), L'C');
		...
		bHasCollided = fBirdPosition < 2 || 
			fBirdPosition > ScreenHeight() - 2 ||
			m_bufScreen[(int)(fBirdPosition + 0)* ScreenWidth() + nBirdX].Char.UnicodeChar != L'C' ||
			m_bufScreen[(int)(fBirdPosition + 1)* ScreenWidth() + nBirdX].Char.UnicodeChar != L'C' ||
			m_bufScreen[(int)(fBirdPosition + 0)* ScreenWidth() + nBirdX + 6].Char.UnicodeChar != L'C' ||
			m_bufScreen[(int)(fBirdPosition + 1)* ScreenWidth() + nBirdX + 6].Char.UnicodeChar != L'C';
